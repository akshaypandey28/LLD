...
int main() {
    ...
    // 2) A New User comes on Platform
    User* user = new User("Aditya", 1.0, 1.0);
    cout <<"\nUser with name " << user->name<< " comes on platform" << endl;

    // 3) Show all available items via Zepto for Aditya
    ZeptoHelper::showAllItems(user);

    // 4) User(Aditya) adds items to cart (some not in a single store)
